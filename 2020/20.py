...
def is_admissible(tiles_: Dict[TileId, Tile], puzzle_: Dict[Position, TileId], candidate_: Tile, position_: Position) -> bool:
...
def remove_from(open_: Dict[Position, Set[TileId]], tileId_: int) -> Dict[Position, Set[TileId]]:
	""" Remove a tile ID from all the possibilities in a dictionnary and returns the result. """
	cleaned = open_.copy()
...
def compute_next_pos_dict(size_: int) -> Tuple[Dict[Position,Position], Position]:
	...
	while not done:
		...
		border_reached = new_position[0]==size_ or new_position[1]==size_ or new_position[0]<0 or new_position[1]<0
...
def solve_jigsaw(tiles_: Dict[TileId, Tile], open_: Dict[Position, Set[TileId]], puzzle_: Dict[Position, TileId], next_pos_: Position, get_next_pos_: Dict[Position,Position], end_position_: Position) -> Tuple[Dict[Position, TileId], bool]:
...
def main():
	...
	# define an open jigsaw dictionnary, which contains the possible tiles on each cell
	op_jigsaw = {}
	get_corner = lambda one, two: {c for c in corners if tiles[(c[0],c[1])].matches_[one] == [] and tiles[(c[0],c[1])].matches_[two] == []}
